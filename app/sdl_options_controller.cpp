...
SdlOptionsUpdateEffects update_sdl_options_menu(
    ...
    const OptionsMenuSection section_before = runtime.options_menu.section;
    const int row_before = runtime.options_menu.selected_row;

    if (runtime.options_menu.waiting_for_input) {
    ...
    const OptionsMenuActionResult result = apply_options_menu_action(
        ...
        intent);
    if (runtime.options_menu.section == section_before && runtime.options_menu.selected_row != row_before) {
    ...
    if (result == OptionsMenuActionResult::BindingChanged) {
        ...
        if (options_row_is_axis_invert(section_before, row_before)) {
    ...
    } else if (result == OptionsMenuActionResult::SectionChanged) {
        effects.play_confirm_sound = true;
