...
void maybe_arm_story_post_route_transition(
    ...
    const whacker::app::AppState from_state,
    const whacker::app::StorySceneState& from_story_scene_state,
    const whacker::app::AppState to_state,
    const whacker::app::StorySceneState& to_story_scene_state) {
    if (!trigger_wipe) {
        return;
    }
    const whacker::app::StorySceneState* from_scene_ptr =
        from_state == whacker::app::AppState::StoryScene ? &from_story_scene_state : nullptr;
    const whacker::app::StorySceneState* to_scene_ptr =
        to_state == whacker::app::AppState::StoryScene ? &to_story_scene_state : nullptr;
...
void apply_story_post_route(
    ...
    switch (route) {
        ...
        default:
            ...
            app_state = whacker::app::AppState::StoryHub;
            break;
    }
...
void end_active_or_quick_match(
    ...
    reset_match_flow(match_flow);
    simulation.reset();
    app_state = previous_mode == ActiveMatchMode::Quick ? AppState::QuickMatchSetup : AppState::MainMenu;
}
